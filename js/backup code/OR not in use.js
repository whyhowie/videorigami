...
OR.Command = function (modele) {
  ...
  this.cmdList = [];
  this.currentCmd = [];
  this.iCC = 0; // Index of characters in the currentCommand
  ...
  function execute () {
    ...
    if (toko[iTok] === "d" || toko[iTok] === "define") {
      ...
      while (Number.isInteger(Number(toko[iTok]))) {
        list.push(toko[iTok++]);  //  x++ returns value before incrementing
      }
      console.log(list)
      model.init(list);
    }
    ...
    // Default should not get these
      ...
      || toko[iTok] === "u"
      || toko[iTok] === "co") {
      console.log("Warning unnecessary token :" + toko[iTok] + "\n");
      iTok++;
      return -1;
    } else {
      // Real default : ignore
...
// -- State undo
  ...
  const commandLoop = () => { // arrow function to bind "this"
    // console.log("@commandLoop, pauseDuration is: "+ pauseDuration)
    // console.log("current token: "+ toko[iTok])
  ...
  const anim = () => {
    ...
    // }
    // if (this.state === State.pause) {
    //   // Need fixes - need to save the current animation progress
    //   // Moved this part to below
    //   // pauseStart = new Date().getTime();
    //   return false;
    // }
    ...
    while (toko[iTok] !== "rparent") {
      execute();
      if (iTok === toko.length) {
        console.log("Warning missing command ending!");
        break;
      }
    }
